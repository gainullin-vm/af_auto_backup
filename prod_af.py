...
local_keyPath_prod_af = "/home/ubuntu/.ssh/prod_af_access"
...
def exec(local_KeyPath_remote, host, tag, timecount):
    commandList = [
    ...
    for command in commandList:
        ssh = subprocess.Popen(["ssh", "-o", "StrictHostKeyChecking=no", "-i", local_KeyPath_remote, host, command],
                            shell=False,
...
def execActive(local_KeyPath_remote, host):
    commandList = [
        "docker ps -q"
    ]
    for command in commandList:
            ...
            for element in result:
                ...
                for command in commandList:
                    ssh = subprocess.Popen(
                        ["ssh", "-o", "StrictHostKeyChecking=no", "-i", local_KeyPath_remote, host, command],
                        shell=False,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE)
                    result = re.findall("\w\B\w*", str(ssh.stdout.readlines()))
                    if result == []:
                        ...
                        for tag in result:
                            ...
                            exec(local_KeyPath_remote, host, tag, timecount)

i = 0
while i < 1:
    for host in hostList:
        if host == hostList[0]:
           local_keyPath_remote = local_keyPath_dev02
        if host == hostListp[1]:
           local_keyPath_remote = local_keyPath_prod_af
        execActive(local_keyPath_remote, host)
